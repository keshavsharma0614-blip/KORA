...
kora telemetry --input <generated-run-or-report.json>
```
...
python3 -m kora telemetry --input <generated-run-or-report.json>
```

Use `--offline` to force the mock/demo path for a reproducible local run without OpenAI credentials.
Use `telemetry` on a JSON artifact generated by a prior run or report flow.
